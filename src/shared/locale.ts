...
export interface AppCopy {
  ...
  workflowGenerationPrompt: string
  ...
  workflowRunDeleteUnlockHint: string
  workflowRunDeleteLockHint: string
...
const APP_COPY: Record<AppLocale, AppCopy> = {
  zh: {
    ...
    recoveryRetryRuntime: '按当前模式重新启动',
    ...
    workItemsHint: '这里是你的工作清单：任务、执行过程、成果和需要你处理的事项都会保存在本机，重启后仍能继续查看。',
    ...
    workflowWorkspace: '工作流',
    workflowChooseRun: '选择一条运行记录查看详情',
    workflowNew: '新建工作流',
    workflowDuplicate: '复制工作流',
    workflowDelete: '删除工作流',
    workflowDeleteConfirm: (name) => `确定删除工作流“${name}”？该工作流的执行记录也会一并删除，且无法恢复。`,
    workflowSave: '保存',
    workflowUndo: '撤销',
    workflowRedo: '重做',
    ...
    workflowGenerationPrompt: 'AI 生成提示词',
    workflowCanvas: '画布',
    ...
    workflowFilePath: 'Workflow 工作目录相对路径',
    ...
    workflowUnviewedRuns: (count) => `${count}条结果未读`,
    ...
    workflowRunDeleteUnlockHint: '解锁后删除运行记录不再需要确认。',
    workflowRunDeleteLockHint: '当前已锁定，删除运行记录前会要求确认。',
    ...
    workflowAllowShellFileHint: '运行前需要显式允许；文件路径仍被限制在 Workflow 工作目录内。',
    ...
    storeInstallLogPath: (path) => `详细操作日志：${path}`,
  ...
  en: {
    ...
    recoveryRetryRuntime: 'Restart in current mode',
    ...
    workItemsHint: 'This is your work list: tasks, execution history, deliverables, and items needing your attention stay on this device after restart.',
    ...
    workflowDeleteConfirm: (name) => `Delete workflow “${name}”? Its run records will also be deleted and cannot be recovered.`,
    workflowSave: 'Save',
    workflowUndo: 'Undo',
    workflowRedo: 'Redo',
    ...
    workflowGenerationPrompt: 'AI generation prompt',
    ...
    workflowFilePath: 'Relative path inside the Workflow working directory',
    ...
    workflowUnviewedRuns: (count) => `${count} unread result${count === 1 ? '' : 's'}`,
    ...
    workflowRunDeleteUnlockHint: 'Unlock to delete run history without confirmation.',
    workflowRunDeleteLockHint: 'Locked: deleting run history will ask for confirmation.',
    ...
    workflowAllowShellFileHint: 'Explicit permission is required before a run; file paths remain confined to the Workflow working directory.',
    ...
    storeInstallLogPath: (path) => `Detailed operation log: ${path}`,
