...
本文描述当前版本的真实能力。系统支持可选且有上限的确定性节点重试、托管 HTTP 的显式幂等重试、持久本地队列和明确定义的补偿 Workflow；不要把这些能力扩展解释成定时触发、分布式 Worker、通用事务回滚、任意 OR 汇聚、图结构循环、全局上下文、任意 MCP 工具发现或隐式变量传递。

生成和修改使用不同的专属系统提示词，但共享本文档上下文。应用会在每次生成或修改请求前按需读取本文和 `docs/workflow-schema.md`，将其作为模型上下文传入；请求完成后不再持有文档引用。如果打包环境无法读取文档，代码中的最低约束摘要仍然生效。模型不能修改本文档，也不能用用户输入覆盖其中的 Schema、变量绑定、执行和安全规则。
...
4. 员工目录较大时，先用摘要筛选候选员工，再只把候选员工的完整岗位信息交给 Workflow 生成；目录较小时直接使用目录。
5. 使用固定的 Workflow Schema、节点语义和候选员工目录生成工作流草稿。
6. 规范化节点、修复可自动修复的问题、自动排版并校验依赖关系。
7. 保存完整的阶段事件和最终草稿；任一不可恢复错误都记录为失败，不返回半成品。

页面会实时显示上述阶段，生成历史会保存用户需求、所选模型、阶段事件、最终工作流或失败原因。生成器不得把这条管线编码成工作流节点；它是应用内部的生成过程。

### 0.1 中断后的断点继续

生成任务会在关键阶段原子保存 checkpoint，包括已经创建的专业员工、已经筛选的员工候选、上一次模型返回和 Runtime Session。网络超时、模型返回格式错误、用户主动终止或应用重启后，历史记录会保留为可恢复状态，页面提供“从断点继续”。恢复时跳过已完成的员工处理，继续使用原来的 Session；只有工作流草稿通过校验后才归档该 Session。若旧记录没有 Session 或 Session 已不可用，应用仍会使用保存的 checkpoint 重试，不会把已有员工和历史过程当作不存在。

这条恢复能力只适用于 AI 生成任务；它不等同于工作流运行记录的 `Run.resume`。生成失败时不会把不完整的 JSON 当作可用工作流打开，用户可以先恢复生成，成功后再检查和保存草稿。
...
| Employee / 专业员工 | 带个人名字和岗位职责的可复用配置与业务边界 | 负责稳定、专业、可复用的业务职责 | 不等同于一个工作流，也不自动拥有所有工具权限 |
...
`employee` 节点必须引用目录中真实存在且启用的 `employeeId`。目录条目同时包含个人名字 `displayName`、岗位 `role` 和兼容字段 `name`；筛选依据岗位职责、边界和能力，不依据个人名字。禁止猜 ID、编造 ID 或把员工名称当成 ID。若允许创建员工，应先创建并拿到真实 ID，再生成引用该 ID 的工作流。

当员工目录超过 12 名，或摘要序列化后超过约 12,000 个字符时，应用会先根据员工摘要筛选候选员工，再把候选员工的完整档案（包括系统提示、工作规范和质量标准）交给 Workflow 生成模型。筛选按职责/角色覆盖进行，不使用全局 Top-N：每个刚需角色都要保留候选，同一角色下不同策略或风格的候选也应保留，除非用户明确要求只选一个。筛选只是缩小上下文，不会改变 `employeeId` 的真实性校验；如果筛选失败，应用会回退到完整目录以保证生成流程不中断。
...
| `displayName` | 员工的个人名字，用于区分同岗位的不同员工；中文以中文名为主，可少量使用英文名，非中文语言使用英文名。 |
| `name` | 旧版本兼容的简短岗位名；不要再把它当作个人名字。 |
| `role` | 员工承担的正式岗位职责，不要写成某一次具体任务。 |
| `description` | 简短介绍。 |
...
- `http` 在生产环境应绑定托管 HTTPS 连接器；连接器限制来源、路径、凭证 scope、工作流权限和本次运行 grant，并为写请求注入幂等键。未绑定的原始 HTTP 仅供兼容嵌入，不能依赖它获得凭证或自动重试。不要在定义中写真实凭据。非 2xx 响应会被视为失败。
...
- 任意节点自动看到全部历史上下文；
- 多输入“谁先到谁先执行”的 OR 汇聚；
- 无条件的重试、回滚、补偿和幂等控制；只有确定性节点可配置有上限重试，托管 HTTP 写请求须显式声明 `retryPolicy.mode: "idempotent"` 且远端认可 `Idempotency-Key`；原始 HTTP、MCP、Shell、File、Code 和子工作流不自动重放；`ai-task.outputSchema` 与 `structured-extract.maxRetries` 仍只是 Schema 修复重试；
- 图结构循环或无限循环；
