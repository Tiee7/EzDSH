...
> **当前实现边界（1.8）**：本文描述的 DSH 对话内调度仍是后续产品方案，并未因为本期工作项开发而完成。当前落地的是开发者模式下的正式工作项入口：员工页和工作流页可以创建任务并启动已有执行器，工作项页负责持续追踪、成果接受和交接；旧的 `employees.run` 仍是即时/调试路径。真正的对话内 Skill 调度、可信外部工具桥、持久异步员工会话和 MCP 接入仍需单独实现与验收。
...
源码依据见本文末尾。现有员工和 External API 的 22 项相关测试在前置评估中通过；1.8 新增的是工作项本地持久化、工作流/员工入口和开发者模式 UI，不应据此宣称已经具备本文的完整员工调度通路。
...
| 员工服务 API | 提供统一的查询、启动、状态、结果和取消能力（本文目标；当前 1.8 尚未提供完整对话调度 API） |
