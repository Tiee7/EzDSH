...
- 当前锁定 `@deepseek-ai/dsh@0.1.5-alpha.1`，源码子模块提交和构建结果必须可追溯；
...
dshRuntimeVersion: 0.1.5-alpha.1
dataSchemaVersion: 1
```
...
```text
工作流 = 事情如何完成
专业员工 = 哪个人以哪个岗位对专业环节负责；档案同时保存个人名字和岗位
智能处理 = 当前流程中的轻量临时推理
技能 = 员工或节点可以调用的原子能力
任务 = 一次业务请求
运行 = 任务执行产生的技术记录
...
运行记录保存在 `<userData>/state/workflow-runs.json`，工作流定义保存在 `<userData>/state/workflows.json`；Workflow 业务文件保存在 `<userData>/workflow/`，并由 Recovery 快照的 `workflow/` 一并覆盖。应用运行期间由单个本地 Worker 从持久队列抢占任务并续租，记录带有可选的幂等键和节点副作用状态。应用重启会回收旧租约：没有外部副作用的运行重新排队，已派发或确认但未完成 checkpoint 的副作用会暂停并要求人工核对，不能静默重放。只有调用方显式提供相同幂等键时，才会对同一工作流 revision 去重。AI 生成任务还会保存阶段 checkpoint 和生成 Session；网络或格式错误导致生成终止时，历史记录提供“从断点继续”，跳过已完成步骤并复用已有产物，成功校验后才归档 Session。
...
- 凭证只通过主进程加密凭证库和元数据 IPC 管理；Workflow JSON、运行记录、诊断日志和 Renderer 不得出现明文密钥或认证请求头。
- 托管 HTTP 连接器把 HTTPS 来源、允许路径、凭证 scope、工作流权限和本次运行 grant 取交集；未绑定托管连接器的原始 URL HTTP 仅兼容旧嵌入，生产默认禁止。
- 重试仅限有上限的确定性节点；托管 HTTP 写请求必须显式声明幂等模式并使用远端认可的 `Idempotency-Key`。MCP、Shell、File、Code、子工作流和未知副作用不自动重放，补偿只能执行定义中声明的反向 Workflow。

### 9.3 专业员工与兼容迁移

专业员工是一份可复用的岗位档案，包含个人名字 `displayName`、正式岗位 `role`、旧版兼容短名称 `name`、业务边界、工作原则、执行规范、质量标准、能力、技能 ID 和档案版本。员工界面统一显示“名字（岗位）”，工作流画布员工节点右上角显示个人名字；用户可随时修改个人名字。员工不拥有内部工作流；同一员工可以被多个工作流引用。旧 Employee 的启用步骤会迁移为非执行性的执行规范，旧 Workflow 的 `agent` 节点会迁移为 `ai-task`。

### 9.4 工作流 JSON 交换

工作流编辑器支持导出、工作流列表支持导入。交换文件使用版本化 JSON 包络，导入前必须完成格式、版本、节点、连线和配置校验；业务模板以 JSON 文件或插件分发，不在页面内置侵入式按钮。

当前版本支持手动运行、人工审批、应用进程内的本地持久队列/单 Worker、有限重试、显式幂等键、明确的补偿 Workflow、凭证引用、托管 HTTP 连接器，以及本地的工作流发布、回滚、客户环境和健康观测；不包含定时/事件触发、24 小时后台或分布式 Worker、预算告警、外部平台自动发布、员工长期记忆或 EZDSH 自我升级。后续无人值守能力仍必须建立在不可变发布版本、分布式任务协调、权限预算、告警、重试补偿和幂等机制之上。
