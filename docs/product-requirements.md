...
当前版本支持手动运行、人工审批、应用进程内的本地持久队列/单 Worker、有限重试、显式幂等键、明确的补偿 Workflow、凭证引用和托管 HTTP 连接器；不包含定时/事件触发、24 小时后台或分布式 Worker、工作流发布流程、通用事务回滚、预算告警、外部平台自动发布、员工长期记忆或 EZDSH 自我升级。后续无人值守能力仍必须建立在不可变发布版本、分布式任务协调、权限预算、告警、重试补偿和幂等机制之上。
