...
> **让每个人都有一个属于自己的 AI 工作与生活入口，把前沿技术变成可直接使用、可组合、可持续成长的能力。**

### 价值观

- **不作恶**：技术始终服务于人，不利用信息差、技术差或用户信任牟利，不以牺牲用户的安全、隐私和自主权为代价。
- **价值先于技术**：以解决真实问题、创造真实价值为标准，而不是炫技。
- **人人都能使用**：不要求用户先成为技术专家，复杂能力应该被自然地使用。
- **开放而可组合**：模型、插件、工具和工作流可以自由连接、替换和扩展。
...
4. `npm ci` 会安装锁定版本的已发布 `@deepseek-ai/dsh@0.1.5-alpha.1` Runtime 及其生产依赖闭包，不需要额外安装或 staging DSH workspace。
