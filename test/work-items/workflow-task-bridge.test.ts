...
describe('WorkflowTaskBridge', () => {
  it('starts a fixed workflow revision with trusted task association and stable command identity', async () => {
    const workflowRuns = { start: vi.fn().mockResolvedValue(run()), resume: vi.fn(), resumeExpected: vi.fn(), cancel: vi.fn(), cancelForWorkTask: vi.fn(), get: vi.fn(), approveExpected: vi.fn(), answerExpected: vi.fn(), findByIdempotencyKey: vi.fn() }
  ...
  it('resumes and cancels the original run and finds linkage by command', async () => {
    ...
    const workflowRuns = { start: vi.fn(), resume: vi.fn().mockResolvedValue(original), resumeExpected: vi.fn().mockResolvedValue(original), cancel: vi.fn().mockResolvedValue(original), cancelForWorkTask: vi.fn().mockResolvedValue(original), get: vi.fn().mockReturnValue(original), approveExpected: vi.fn(), answerExpected: vi.fn().mockResolvedValue(original), findByIdempotencyKey: vi.fn().mockReturnValue(original) }
    const bridge = new WorkflowTaskBridge(workflowRuns)
    ...
    expect(await bridge.cancelTask(original.id)).toBe(original)
    ...
    expect(workflowRuns.cancelForWorkTask).toHaveBeenCalledWith(original.id)
  ...
  it('keeps debug starts outside WorkTask association', async () => {
    const workflowRuns = { start: vi.fn().mockResolvedValue(run({ debug: true })), resume: vi.fn(), resumeExpected: vi.fn(), cancel: vi.fn(), cancelForWorkTask: vi.fn(), get: vi.fn(), approveExpected: vi.fn(), answerExpected: vi.fn(), findByIdempotencyKey: vi.fn() }
    const bridge = new WorkflowTaskBridge(workflowRuns)
