...
async function fixture(localPackage: { packageJson?: string }): Promise<{
  ...
  await writeFile(runtimePackagePath, JSON.stringify({ name: '@deepseek-ai/dsh', version: '0.1.6-alpha.1' }))
...
describe('profile module repair', () => {
  ...
  it('leaves a profile copy from the selected Runtime untouched', async () => {
    const current = await fixture({ packageJson: JSON.stringify({ name: '@deepseek-ai/dsh-llm', version: '0.1.6-alpha.1' }) })
