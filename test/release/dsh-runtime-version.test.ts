...
describe('DSH Runtime version', () => {
  it('uses the exact vendored source Runtime pin', () => {
    expect(PINNED_DSH_RUNTIME_VERSION).toBe('0.1.6-alpha.2')
    expect(PINNED_DSH_SOURCE_COMMIT).toBe('ddefc45fbc7f8e46dd73185e68295696d1297887')
  })
  ...
  it('rejects a vendored checkout that differs from the source commit pin', () => {
    expect(() => assertPinnedDshSourceCommit('fixture', '141eb6fef83422698aef7a981029e843e8161534'))
      .toThrow(/fixture.*ddefc45f.*141eb6fe/)
  })
  ...
  it('rejects a version that differs from the pin with useful details', () => {
    expect(() => assertPinnedDshRuntimeVersion('fixture', '0.1.0-rc.8'))
      .toThrow(/fixture.*0\.1\.6-alpha\.2.*0\.1\.0-rc\.8/)
  })

  it('accepts the pinned version', () => {
    expect(() => assertPinnedDshRuntimeVersion('fixture', '0.1.6-alpha.2')).not.toThrow()
  })
  ...
  it('verifies the authenticated token URL with slash-style workspace and session RPCs', async () => {
    ...
    try {
      ...
      await mkdir(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh-agent'), { recursive: true })
      ...
      await mkdir(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh-persona'), { recursive: true })
      await mkdir(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh-system-prompt'), { recursive: true })
      ...
      await writeFile(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh', 'package.json'), JSON.stringify({ name: '@deepseek-ai/dsh', version: '0.1.6-alpha.2' }))
      ...
      await writeFile(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh-persona', 'index.js'), 'module.exports = {}\n')
      await writeFile(join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh-system-prompt', 'index.js'), 'module.exports = {}\n')
...
`)
  ...
  it('rejects a selected Runtime entry whose owning manifest is stale before Runtime startup', async () => {
    ...
    try {
      ...
      expect(result.stderr).toMatch(/selected DSH Runtime.*0\.1\.6-alpha\.2.*0\.1\.0-rc\.8/)
    } finally {
      await rm(bundleRoot, { recursive: true, force: true })
    }
  })
  ...
  it('rejects a selected Runtime entry from the previous pin before Runtime startup', async () => {
    ...
    try {
      ...
      await writeFile(
        join(bundleRoot, 'node_modules', '@deepseek-ai', 'dsh', 'package.json'),
        JSON.stringify({ name: '@deepseek-ai/dsh', version: '0.1.6-alpha.1' })
      )
      await writeFile(nodeExecutable, '')
      await writeFile(pnpmExecutable, '')
      ...
      const result = spawnSync(process.execPath, [verifier, bundleRoot], { encoding: 'utf8' })
      expect(result.status).not.toBe(0)
      expect(result.stderr).toMatch(/selected DSH Runtime.*0\.1\.6-alpha\.2.*0\.1\.6-alpha\.1/)
    } finally {
      await rm(bundleRoot, { recursive: true, force: true })
    }
  })
